...
const PriceAlert: React.FC<PriceAlertProps> = ({ currentAlert, onSetAlert, onRemoveAlert }) => {
  ...
  if (!isEditing && currentAlert) {
    return (
        <div className="mt-4 p-3 bg-slate-100 dark:bg-slate-700/50 rounded-lg flex flex-col sm:flex-row items-center justify-between gap-2 animate-fade-in">
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-4 flex flex-col sm:flex-row gap-2 items-center p-3 bg-slate-100 dark:bg-slate-700/50 rounded-lg animate-fade-in">
      ...
      <div className="relative flex-grow w-full sm:w-auto">
         ...
         <input
            ...
            className="w-full pl-7 pr-3 py-2 bg-white dark:bg-slate-600 border border-slate-300 dark:border-slate-500 rounded-md focus:outline-none focus:ring-2 focus:ring-primary"
            min="0"
            step="any"
            required
         />
      </div>
      <button type="submit" className="w-full sm:w-auto px-4 py-2 bg-secondary text-white font-semibold rounded-md hover:bg-emerald-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-secondary dark:focus:ring-offset-dark transition-colors duration-200">
        Set Alert
      </button>
      {currentAlert && <button type="button" onClick={() => setIsEditing(false)} className="w-full sm:w-auto px-4 py-2 text-slate-600 dark:text-slate-300 font-semibold rounded-md hover:bg-slate-200 dark:hover:bg-slate-600">Cancel</button>}
    </form>
  );
};
